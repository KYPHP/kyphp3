...
final class KYPHP {
	...
	public static function RUN($config=array()){
		...
		if($conf->get('safe_key')){
			$KYPHP->safe = new zlsafe($conf->get('safe_key'));
		}else{
			$KYPHP->safe = new zlsafe();
		}
		$KYPHP->rsa = new zlrsa();
		...
		$k=isset($_GET['k'])?$_GET['k']:DEFAULT_MODULE;
		 ...
		 if(!defined('__ROOT__'))define('__ROOT__', APP_PATH);
		 ...
		 $DEFAULT_S_PATH=$conf->get('DEFAULT_S_PATH');
		 if($DEFAULT_S_PATH){define('DEFAULT_S_PATH',APP_DIR.'/'.$DEFAULT_S_PATH.'/');}else{define('DEFAULT_S_PATH',APP_DIR.'/service/');}
		...
		$KYPHP->language = new Language($code);
		 ...
		 if($KYPHP->config->get('waf_report')==''||$KYPHP->config->get('waf_report')!=false){
			 $waf=new zlwaf();			 
		 }
		...
		if($KYPHP->config->get('templete_mode')=='smarty'){
			...
			if(file_exists(KYPHP_PATH.'/'.$smartyClass)){
				require KYPHP_PATH.'/'.$smartyClass;
				require_once KYPHP_PATH.'/kyphp_extras/Smarty.php';
	...
	static function autoload_APP($class,$locdirs){
		...
		if(isset($config['DEFAULT_S_PATH'])){$config['DEFAULT_S_PATH']=$config['DEFAULT_S_PATH'].DIRECTORY_SEPARATOR;}else{
			$config['DEFAULT_S_PATH']='service'.DIRECTORY_SEPARATOR;
		}
		...
		if($locdirs){
			...
			if(isset($config['DEFAULT_S_PATH']))
			{
				$controlclassfile=APP_PATH.DIRECTORY_SEPARATOR.$config['app_dir'].$config['DEFAULT_S_PATH'].'/'.str_ireplace('Action','',strtolower($class)).'.class.php';	
		...
		$actionclassfile=APP_PATH.DIRECTORY_SEPARATOR.$config['app_dir'].$config['DEFAULT_S_PATH'].str_ireplace('Service','',$class).'.class.php';
