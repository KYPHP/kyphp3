...
class Action {
	...
	public function service($file){
	 ...
	 if(file_exists(DEFAULT_S_PATH."/{$file}.php"))require_once DEFAULT_S_PATH."/{$file}.php";
		$classname = 'Service' . preg_replace('/[^a-zA-Z0-9]/', '', $file);
		
	 if(!class_exists($classname))error(2,$classname);
	 
	 $this->S[$file]=new $classname($this->kyphp,$this->kyphp_system_k);
	 return  $this->S[$file];
	}
	...
	if(!is_file($file)){
		trigger_error('Error: Could not load templete file ' . $file);
	}
	if($this->config->get('templete_mode')=='smarty'){
		if(is_file($file)){
			...
			if(strstr($content,'<kyphp>')){
				$content=str_replace(array('<kyphp>','</kyphp>'),array('{',';}'),$content);
				$html=new Html();
				$html->data=$this->data;
				$html->config=$this->config;
				$html->url=$this->url;
				$content=$html->make($content);	
					
			}
			if($this->config->get('smarty_phpexe')==true||$this->config->get('smarty_phpexe')=='')
	...
	public function label($name,$txt){
		global $lable;		
		$lable[$name]=$txt;	
		$this->data['kysmarty']['lable']=$lable;
		$this->data[$name]=$txt;
